...
class DouyinDownloader(Downloader):
    ...
    def __init__(self, cookie=None):
        ...
        logger.info("DouyinDownloader initialized, cookie=%s", self._mask_cookie(self.headers_config.get("Cookie")))
    ...
    def fetch_video_info(self, video_url: str) -> json:
        try:
            ...
            logger.info("Fetching douyin video info, aweme_id=%s", aweme_id)

            response = requests.get(full_url, headers=kwargs)
            return response.json()
        except Exception as e:
            logger.error("Douyin video info request failed: %s", e)
    ...
    def download(
        ...
        try:
            logger.info("Downloading douyin audio, url=%s, output_dir=%s, quality=%s", video_url, output_dir, quality)
    ...
    def download_video(self, video_url: str, output_dir: Union[str, None] = None) -> str:
        ...
        try:
            ...
            logger.error("Douyin video download failed: %s", e)
            raise ValueError("请求失败:", e)
